...
function OrionLib:MakeWindow(WindowConfig)
	...
	local SizeSlider = SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(40, 40, 40), 0, 5), {
		...
		AddThemeObject(SetProps(MakeElement("Label", "Size", 13), {
	...
	local TransparencySlider = SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(40, 40, 40), 0, 5), {
		Size = UDim2.new(0, 150, 0, 25),
		Position = UDim2.new(1, -240, 0, 42),
		BackgroundTransparency = 0.5
	}), {
		AddThemeObject(SetProps(MakeElement("Label", "Trans", 13), {
			Size = UDim2.new(0, 35, 1, 0),
			Position = UDim2.new(0, 8, 0, 0),
			Font = Enum.Font.GothamSemibold,
			Name = "Label"
	...
	local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
		...
		local function UpdateSize(Input)
			if SizeDragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
				local SizeScale = math.clamp((Input.Position.X - SizeSlider.AbsolutePosition.X - 45) / (SizeSlider.AbsoluteSize.X - 45), 0, 1)
				local NewSize = UDim2.new(0, 500 + (SizeScale * 300), 0, 300 + (SizeScale * 200))
				MainWindow.Size = NewSize
				SizeSlider.Slider.Size = UDim2.new(SizeScale, 0, 1, 0)
			end
		end
		...
		local function UpdateTransparency(Input)
			if TransDragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
				local TransScale = math.clamp((Input.Position.X - TransparencySlider.AbsolutePosition.X - 45) / (TransparencySlider.AbsoluteSize.X - 45), 0, 1)
				local NewTransparency = TransScale * 0.5
				
				-- Update all elements' transparency
				for _, v in pairs(MainWindow:GetDescendants()) do
					if v:IsA("Frame") or v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("ImageLabel") or v:IsA("ImageButton") then
						if v ~= SizeSlider and v ~= TransparencySlider and v ~= SizeSlider.Slider and v ~= TransparencySlider.Slider then
							local CurrentTransparency = v.BackgroundTransparency
							if CurrentTransparency < 1 then
								v.BackgroundTransparency = math.min(CurrentTransparency + NewTransparency, 1)
							end
							if v:IsA("TextLabel") or v:IsA("TextButton") then
								v.TextTransparency = NewTransparency
							end
							if v:IsA("ImageLabel") or v:IsA("ImageButton") then
								v.ImageTransparency = NewTransparency
							end
						end
					end
				end
				
				TransparencySlider.Slider.Size = UDim2.new(TransScale, 0, 1, 0)
			end
		end
