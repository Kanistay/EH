...
function OrionLib:MakeWindow(WindowConfig)
	...
	local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
		...
		SetChildren(SetProps(MakeElement("TFrame"), {
			...
			AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 7), {
				Size = UDim2.new(0, 70, 0, 25), -- Reduced height
	...
	-- Create bottom bar outside main window
	local BottomBar = SetChildren(SetProps(MakeElement("TFrame"), {
		Parent = Orion,
		Size = UDim2.new(0, 615, 0, 30),
		Position = UDim2.new(0.5, -307, 0.5, 172), -- Position below main window
		BackgroundTransparency = 1
	}), {
		-- Size Slider
	...
	local function UpdateSize(Input)
		if SizeDragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
			local SizeScale = math.clamp((Input.Position.X - BottomBar:FindFirstChild("Slider").AbsolutePosition.X - 35) / (BottomBar:FindFirstChild("Slider").AbsoluteSize.X - 35), 0, 1)
			...
			TweenService:Create(BottomBar:FindFirstChild("Slider").Slider, TweenInfo.new(0.1), {Size = UDim2.new(SizeScale, 0, 1, 0)}):Play()
		end
	end

	BottomBar:FindFirstChild("Slider").InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			SizeDragging = true
		end
	end)

	BottomBar:FindFirstChild("Slider").InputEnded:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			SizeDragging = false
		end
	end)
	...
	local function UpdateTransparency(Input)
		if TransDragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
			local TransScale = math.clamp((Input.Position.X - BottomBar:FindFirstChild("TransparencySlider").AbsolutePosition.X - 35) / (BottomBar:FindFirstChild("TransparencySlider").AbsoluteSize.X - 35), 0, 1)
			...
			TweenService:Create(BottomBar:FindFirstChild("TransparencySlider").Slider, TweenInfo.new(0.1), {Size = UDim2.new(TransScale, 0, 1, 0)}):Play()
		end
	end

	BottomBar:FindFirstChild("TransparencySlider").InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			TransDragging = true
		end
	end)

	BottomBar:FindFirstChild("TransparencySlider").InputEnded:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			TransDragging = false
		end
	end)
	...
	-- Update bottom bar position when window moves
	AddConnection(MainWindow:GetPropertyChangedSignal("Position"), function()
		BottomBar.Position = UDim2.new(0, MainWindow.Position.X.Offset, 0, MainWindow.Position.Y.Offset + MainWindow.Size.Y.Offset + 5)
	end)

	-- Update bottom bar size when window resizes
